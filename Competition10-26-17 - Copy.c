...
//SECTION ONE, GRAB FIRST CONE 4000 milliseconds
//	autonStationary();
	crab(-70,800); //close claw
	drivestraight(70, 1500); //drive forward
	pointturn(-70,450);//Turn right
	drivestraight(70,1500);//Forwards
	crab(60, 1000);
	pointturn(70,950);//Turn Left
	driveside(70,300);
	crab(-60, 1300);//Close claw on cone
	crab(-20,0);//Keep claw closed on cone

	//SECTION TWO, MOVE AND SCORE FIRST CONE(2 points) 5200 milliseconds
	drivestraight(-70, 800);//Backwards just a bit
	driveside(-70, 2000);//Crabwalk left
	lift(-70, 1000);//Lift up
	drivestraight(70,500);//Forwards
	lift(70,400);//Lift down to place cone on stationary goal
	crab(70,500);//Open claw to drop on stationary goal.
...
//	autonStationary();
	...
	drivestraight(70,1500); //drive to stationary goal
	lift(70,100); //lift cone
	lift(20,0); //hold
	pointturn(70,400); //turn so cone is above stationary goal
	lift(-70,400); //put cone on stationary goal
	crab(70,600);//let go of cone
